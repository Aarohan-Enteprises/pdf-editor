...
export default function UnlockPdfPage() {
  ...
  const [outputFilename, setOutputFilename] = useState('unlocked-document');
  ...
  const handleUnlock = useCallback(async () => {
    ...
    try {
      ...
      const response = await new Promise<Response>((resolve, reject) => {
        ...
        xhr.open('POST', `${apiUrl}/api/unlock`);
      ...
      const data = new Uint8Array(await unlockedBlob.arrayBuffer());

      setUnlockedData(data);
      setProcessingStage('done');
    } catch (error) {
      console.error('Unlock failed:', error);
      setUploadError(error instanceof Error ? error.message : 'Failed to unlock PDF');
  ...
  const clearFile = useCallback(() => {
    ...
    setUploadError(null);
    setPassword('');
    setOutputFilename('unlocked-document');
    setProcessingStage('idle');
  }, []);

  const handlePreview = useCallback(() => {
    if (unlockedData) {
      const filename = outputFilename.trim() || 'unlocked-document';
      showPreview(unlockedData, `${filename}.pdf`);
    }
  }, [unlockedData, outputFilename, showPreview]);

  const handleDownload = useCallback(() => {
    if (unlockedData) {
      const blob = new Blob([new Uint8Array(unlockedData)], { type: 'application/pdf' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${outputFilename.trim() || 'unlocked-document'}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    }
  }, [unlockedData, outputFilename]);
      ...
      <div className="w-full px-6 lg:px-12 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="w-14 h-14 rounded-2xl bg-yellow-100 dark:bg-yellow-950/30 text-yellow-600 dark:text-yellow-400 flex items-center justify-center">
          ...
          <div className="space-y-4">
            ...
            {file && (
              <div className="card p-4 space-y-4">
                ...
                >
                  {isProcessing ? t('unlocking') : t('unlock')}
                </button>
              </div>
            )}
          </div>
            ...
            {!isProcessing && !unlockedData && (
              <div className="card p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                  {file ? 'Ready to unlock' : 'No PDF selected'}
                </h3>
                <p className="text-gray-600 dark:text-gray-400">
                  {file
                    ? 'Enter the PDF password and click Unlock PDF'
              ...
              <div className="flex gap-3">
                ...
                <div className="text-sm text-slate-600 dark:text-slate-300">
                  ...
                  <p className="text-slate-500 dark:text-slate-400">
                    Your file is encrypted during transfer, processed instantly, and automatically deleted after decryption.
                  </p>
                </div>
              </div>
            </div>
